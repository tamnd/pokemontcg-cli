// Package cli assembles the pokemontcg command tree from the pokemontcg
...
// NewApp assembles the kit application from the pokemontcg domain. The
...
//
// To add a command, declare it in pokemontcg/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := pokemontcg.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(pokemontcg.Domain{}).Register(app)
