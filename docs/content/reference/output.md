...
describes what users get. Pick a format with `-o`, or let pokemontcg choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pokemontcg <command> -o table   # aligned columns for reading
pokemontcg <command> -o jsonl   # one JSON object per line, for piping
pokemontcg <command> -o json    # a single JSON array
pokemontcg <command> -o csv     # spreadsheet friendly
pokemontcg <command> -o tsv     # tab-separated
pokemontcg <command> -o url     # just the URL column
pokemontcg <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pokemontcg <command> --fields id,title,url
```
...
```bash
pokemontcg <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pokemontcg <command>            # a table, because this is a terminal
pokemontcg <command> | wc -l    # JSONL, because this is a pipe
```
